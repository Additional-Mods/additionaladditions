...
public class AAEntities {
    public static final Supplier<EntityType<GlowStickEntity>> GLOW_STICK =
            AARegistries.ENTITY_TYPES.register(
                    Identifier.tryBuild(AdditionalAdditions.NAMESPACE, "glow_stick"),
                    () -> EntityType.Builder.of(
                                    (EntityType<GlowStickEntity> type, Level level) -> new GlowStickEntity(type, level),
                                    MobCategory.MISC
                            )
                            .sized(0.25f, 0.25f).updateInterval(20)
                            .clientTrackingRange(4).build(ResourceKey.create(Registries.ENTITY_TYPE, Identifier.tryBuild(AdditionalAdditions.NAMESPACE, "glow_stick")))
            );

    public static final Supplier<EntityType<RopeArrow>> ROPE_ARROW =
            AARegistries.ENTITY_TYPES.register(
                    Identifier.tryBuild(AdditionalAdditions.NAMESPACE, "rope_arrow"),
                    () -> EntityType.Builder.of(
                                    (EntityType<RopeArrow> type, Level level) -> new RopeArrow(type, level),
                                    MobCategory.MISC
                            )
                            .sized(0.5f, 0.5f).eyeHeight(0.13f).updateInterval(20)
                            .clientTrackingRange(4).build(ResourceKey.create(Registries.ENTITY_TYPE, Identifier.tryBuild(AdditionalAdditions.NAMESPACE, "rope_arrow")))
            );
