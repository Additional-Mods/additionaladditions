...
public class AdditionalEntities {
    public static final Supplier<EntityType<GlowStickEntity>> GLOW_STICK_ENTITY_ENTITY_TYPE =
            Registrar.entityType(ResourceLocation.tryBuild(AdditionalAdditions.NAMESPACE, "glow_stick_entity"), () ->
                    EntityType.Builder.of(
                                    (EntityType<GlowStickEntity> type, Level level) -> new GlowStickEntity(type, level),
                                    MobCategory.MISC
                            )
                            .sized(0.25f, 0.25f).updateInterval(4).clientTrackingRange(10).build("glow_stick_entity")
            );
