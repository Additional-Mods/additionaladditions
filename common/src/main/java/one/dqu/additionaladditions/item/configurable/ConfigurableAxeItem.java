...
public class ConfigurableAxeItem extends AxeItem {
    ...
    public ConfigurableAxeItem(Tier tier, Properties properties, Consumer<DataComponentMap.Builder> configurer) {
        super(tier, properties);
        this.configurer = configurer;
    }

    public ConfigurableAxeItem(Tier tier, Properties properties, Supplier<ToolItemConfig> configSupplier) {
        super(tier, properties);
        this.configurer = builder -> {
            ToolItemConfig config = configSupplier.get();
            builder.set(DataComponents.MAX_DAMAGE, config.durability());
            builder.set(DataComponents.ATTRIBUTE_MODIFIERS, AxeItem.createAttributes(tier, config.attackDamage(), config.attackSpeed()));
            builder.set(DataComponents.TOOL, config.toolProperties(tier, BlockTags.MINEABLE_WITH_AXE));
        };
    }
