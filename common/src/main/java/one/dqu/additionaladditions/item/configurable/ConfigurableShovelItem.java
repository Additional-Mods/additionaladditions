...
public class ConfigurableShovelItem extends ShovelItem {
    ...
    public ConfigurableShovelItem(Tier tier, Properties properties, Supplier<ToolItemConfig> configSupplier) {
        super(tier, properties);
        this.configurer = builder -> {
            ToolItemConfig config = configSupplier.get();
            builder.set(DataComponents.MAX_DAMAGE, config.durability());
            builder.set(DataComponents.ATTRIBUTE_MODIFIERS, ShovelItem.createAttributes(tier, config.attackDamage(), config.attackSpeed()));
            builder.set(DataComponents.TOOL, config.toolProperties(tier, BlockTags.MINEABLE_WITH_SHOVEL));
        };
    }
