...
public class RopeArrowTests {
    ...
    public static void gravityBlockDupe(GameTestHelper ctx) {
        ...
        ctx.runAtTickTime(80, () -> {
            int total = 0;
            for (ItemEntity item : ctx.getEntities(EntityTypes.ITEM)) {
            ...
            ctx.assertTrue(total <= 8, Component.literal("Expected at most 8 total rope (items + placed), got " + total));
            ctx.succeed();
        });
    }
